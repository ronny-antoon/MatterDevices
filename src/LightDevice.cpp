...
LightDevice::LightDevice(char *device_name, gpio_num_t light_pin, gpio_num_t button_pin, esp_matter::endpoint_t *aggregator)
    : BaseDevice()
{
    ...
    else
    {
        ...
        ESP_LOGI(__FILENAME__, "Creating LightAccessory with light_pin: %d, button_pin: %d", light_pin, button_pin);
        ...
        lightAccessory->setReportAttributesCallback([](void *self)
                                                    { static_cast<LightDevice *>(self)->reportEndpoint(); }, this);
    }
...
bool LightDevice::getAccessoryPowerState()
{
    return lightAccessory->getPower();
}

void LightDevice::setAccessoryPowerState(bool powerState)
{
    lightAccessory->setPower(powerState);
}
...
bool LightDevice::getEndpointPowerState()
{
    ...
    esp_matter::attribute_t *on_off_attribute = esp_matter::attribute::get(on_off_cluster, chip::app::Clusters::OnOff::Attributes::OnOff::Id);
