...
PlugInDevice::PlugInDevice(char *device_name, gpio_num_t relay_pin, gpio_num_t button_pin, esp_matter::endpoint_t *aggregator)
    : BaseDevice()
{
    ...
    else
    {
        ...
        ESP_LOGI(__FILENAME__, "Creating PlugInAccessory with relay_pin: %d, button_pin: %d", relay_pin, button_pin);
        ...
        accessory->setReportAttributesCallback([](void *self)
                                               { static_cast<PlugInDevice *>(self)->reportEndpoint(); }, this);
    }
...
bool PlugInDevice::getAccessoryPowerState()
{
    return accessory->getPower();
}

void PlugInDevice::setAccessoryPowerState(bool powerState)
{
    accessory->setPower(powerState);
}
