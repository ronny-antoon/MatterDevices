...
FanDevice::FanDevice(char *device_name, gpio_num_t fan_pin, gpio_num_t button_pin, esp_matter::endpoint_t *aggregator)
    : BaseDevice()
{
    ...
    else
    {
        ...
        fanAccessory->setReportAttributesCallback([](void *self)
                                                  { static_cast<FanDevice *>(self)->reportEndpoint(); }, this);
    }
...
bool FanDevice::getAccessoryPowerState()
{
    return fanAccessory->getPower();
}

void FanDevice::setAccessoryPowerState(bool powerState)
{
    fanAccessory->setPower(powerState);
}

bool FanDevice::getEndpointPowerState()
{
    esp_matter::cluster_t *fan_cluster = esp_matter::cluster::get(
        endpoint, chip::app::Clusters::FanControl::Id);
